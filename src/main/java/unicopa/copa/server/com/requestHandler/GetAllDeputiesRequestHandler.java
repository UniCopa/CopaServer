 ...
 */
public class GetAllDeputiesRequestHandler extends RequestHandler {

    public GetAllDeputiesRequestHandler(CopaSystemContext context) {
	super(context);
    }
	...
	GetAllDeputiesRequest req = (GetAllDeputiesRequest) request;
	checkEventPermission(userID, req.getEventID(), UserRole.RIGHTHOLDER);
	try {
	    List<String> deputies = getContext().getDbservice().getDeputies(
		    req.getEventID());
	    return new GetAllDeputiesResponse(deputies);
